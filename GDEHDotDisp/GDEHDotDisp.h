...
/********************Commands***********************/
#define GDEH_RESET(GDE) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_SW_RESET_CMD);GDEH_BUSY_CHECK(GDE);}
#define GDEH_INITIAL_CODE(GDE) GDESPI_WRITE_CMD(GDE.SPI,GDEH_INITIAL_CODE_SETTING_CMD);
#define GDEH_MASTER_ACTIVATE(GDE) GDESPI_WRITE_CMD(GDE.SPI,GDEH_MASTER_ACTIVATION_CMD);
#define GDEH_Write_BW(GDE) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_WRITE_BW_CMD);}
#define GDEH_Write_RED(GDE) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_WRITE_RED_CMD);}
#define GDEH_PROGRAM_OTP(GDE) GDESPI_WRITE_CMD(GDE.SPI,GDEH_PRG_VCOM_OTP_CMD)
#define GDEH_WRITE_REG_VCOM_CNTL(GDE) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_WR_VCOM_CNTL_CMD);\
GDESPI_WRITE_DATA(GDE.SPI,0x04);\
GDESPI_WRITE_DATA(GDE.SPI,0x63);}
#define GDEH_AUTO_RED(GDE,_byte) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_AUTO_RED_CMD);GDESPI_WRITE_DATA(GDE.SPI,_byte);}
#define GDEH_AUTO_BW(GDE,_byte) {GDESPI_WRITE_CMD(GDE.SPI,GDEH_AUTO_BW_CMD);GDESPI_WRITE_DATA(GDE.SPI,_byte);}
#define GDEH_TERMINATOR(GDE) GDESPI_WRITE_CMD(GDE.SPI,GDEH_TERMINATOR_CMD )
/************Gate Scanning*****************/
...
void GDEH_INIT(GDEH);
void GDEH_WRITE_VOLTAGE(GDEH,GDEH_GATE_VOLTAGE);
void GDEH_WRITE_SOURCE_VOLTAGE(GDEH,float,float);
void GDEH_WRITE_REG_INI_CODE(GDEH);
void GDEH_BUSY_CHECK(GDEH);
uint8_t GDEH_READ_REG(GDEH);
void GDEH_BOOST_START(GDEH,GDEH_SOFT_START);
void GDEH_GO_SLEEP(GDEH,GDEH_DEEP_SLEEP);
void GDEH_ENTRYMODE_SETTINGS(GDEH,GDEH_ENTRY_SEQ);
void GDEH_VCI_DETECTION(GDEH,GDEH_VCI_DETECT);
void GDEH_TEMP_CNTL(GDEH,uint16_t);
void GDEH_UPD_CNT(GDEH,GDEH_Display_Update);
void GDEH_VCOM_SENSE_Duration(GDEH,uint8_t);
uint8_t GDEH_WRITE_VCOM(GDEH, float);
void GDEH_WRITE_REGS_FOR_DISP_OPT_USER(GDEH,uint8_t* , uint8_t);
void GDEH_WAVEFORM_CTL(GDEH,GDEH_WFM_CTL );
void GDEH_SET_POSITION(GDEH,uint8_t X_Start, uint8_t X_End, uint16_t Y_Start, uint16_t Y_End);
void GDEH_SET_ADDRESS_CNTR(GDEH,uint8_t ADRX, uint16_t ADRY);
void GDEH_UPDATE(GDEH);
void GDEH_WRITE(GDEH,uint8_t *BLACK_WHITE, uint8_t* RED, int SIZE, uint8_t INVERSE);
void GDEH_CLEAR(GDEH,int SIZE);
